...
surt@meta.data$cell_type <- factor(colData(pdacs_CB)$cell_type)
...
surt <- Spaniel::createSeurat(counts = ssm, barcodeFile = fn,
                              projectName = "PDAC-A", sectionNumber = "1")
surt@images <- metadata(pdac)$images
surt@meta.data$seurat_clusters <- factor(colData(pdacs_GO)$seurat_clusters)
surt <- Seurat::SetIdent(surt, value = "seurat_clusters")
surt <- Seurat::ScaleData(surt)
```
...
surt <- Spaniel::createSeurat(counts = ssm, barcodeFile = fn,
                              projectName = "PDAC-A", sectionNumber = "1")
surt@images <- metadata(pdac)$images
surt@meta.data$seurat_clusters <- factor(colData(pdacs_KG)$seurat_clusters)
surt <- Seurat::SetIdent(surt, value = "seurat_clusters")
surt <- Seurat::ScaleData(surt)
```
...
info <- as.data.frame(surt@meta.data)
info$batch <- NA
...
``` r
surt_st <- surt[, grepl("ST", surt@meta.data[["batch"]])]
mycolor <- c("0" = "purple", "1" = "deepskyblue1", "2" = "grey20",
             "3" = "limegreen", "4" = "red")
...
surt@meta.data[["cell_state"]] <- tmp
surt@meta.data[["cell_state"]][surt@meta.data[["cell_state"]] == 0] <- "Unspecified"
surt@meta.data[["cell_state"]][surt@meta.data[["cell_state"]] == 1] <- "Pancreas-1"
surt@meta.data[["cell_state"]][surt@meta.data[["cell_state"]] == 2] <- "Cancer"
surt@meta.data[["cell_state"]][surt@meta.data[["cell_state"]] == 3] <- "Pancreas-2"
surt@meta.data[["cell_state"]][surt@meta.data[["cell_state"]] == 4] <- "Lymphocyte"
surt@meta.data[["cell_state"]][surt@meta.data[["cell_state"]] == 5] <- "Unspecified"
surt@meta.data[["cell_state"]][surt@meta.data[["cell_state"]] == 6] <- "Unspecified"
```
...
``` r
title <- "PDAC (Seurat)"
labels <- factor(surt@meta.data[["cell_state"]],
                 levels = c("Pancreas-1", "Pancreas-2", "Cancer", "Lymphocyte",
                            "Unspecified"))
