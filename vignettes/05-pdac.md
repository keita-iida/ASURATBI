...
-   [7 Using the existing softwares](#7-using-the-existing-softwares)
    ...
    -   [7.2 ssGSEA using Gene Ontology
        database](#72-ssgsea-using-gene-ontology-database)
    -   [7.3 PAGODA2 using Gene Ontology
        database](#73-pagoda2-using-gene-ontology-database)
...
``` r
mat <- Seurat::GetAssayData(object = surt, slot = "data")
pdac <- Spaniel::createSeurat(counts = mat, barcodeFile = fn,
                              projectName = "PDAC", sectionNumber = "1")
pdac@images <- metadata(pdacst)$images
cond <- c(rep("inDrop", ncol(pdacrna)), rep("ST", ncol(pdacst)))
pdac <- Seurat::AddMetaData(object = pdac, metadata = cond, col.name = "batch")
```
...
``` r
pdac@misc[["getGeneSets"]] <- escape::getGeneSets(species = "Homo sapiens",
                                                  library = "C5")
df <- pdac@misc[["getGeneSets"]]
df <- df[grepl("GOBP", names(df))]
pdac@misc[["getGeneSets"]] <- df
```
...
``` r
set.seed(1)
ES <- escape::enrichIt(obj = pdac, gene.sets = pdac@misc[["getGeneSets"]],
                       groups = 1000, cores = 4)
pdac@misc[["enrichIt"]] <- ES
```

    [1] "Using sets of 1000 cells. Running 3 times."
    Setting parallel calculations through a SnowParam back-end
    with workers=4 and tasks=100.
    Estimating ssGSEA scores for 7487 gene sets.
    ...
    Setting parallel calculations through a SnowParam back-end
    with workers=4 and tasks=100.
    Estimating ssGSEA scores for 7487 gene sets.
...
``` r
mat <- pdac@misc[["enrichIt"]]

# t-SNE
set.seed(1)
res <- Rtsne::Rtsne(mat, dim = 2, pca = TRUE, initial_dims = 50)
pdac@reductions[["tsne_ssgsea"]] <- res$Y

# UMAP
set.seed(1)
res <- umap::umap(mat, n_components = 2)
pdac@reductions[["umap_ssgsea"]] <- res$layout
```

Perform unsupervised clustering of cells using Seurat functions.

``` r
temp <- Seurat::CreateSeuratObject(counts = t(pdac@misc[["enrichIt"]]),
                                   project = "pdac")
temp <- Seurat::ScaleData(temp, features = rownames(temp))
temp <- Seurat::RunPCA(temp, features = rownames(temp))
temp <- Seurat::FindNeighbors(temp, reduction = "pca", dims = seq_len(40))
temp <- Seurat::FindClusters(temp, resolution = 0.15)
pdac <- Seurat::AddMetaData(pdac, metadata = temp$seurat_clusters,
...
``` r
# ssGSEA cluster
labels <- pdac$ssgsea_clusters
mycolor <- c("0" = "cadetblue2", "1" = "limegreen", "2" = "red",
             "3" = "magenta", "4" = "grey20")
df <- as.data.frame(pdac@reductions[["tsne_ssgsea"]])
p <- ggplot2::ggplot() +
  ggplot2::geom_point(ggplot2::aes(x = df[, 1], y = df[, 2], color = labels),
                      size = 0.2, alpha = 1) +
  ggplot2::labs(title = "PDAC (ssGSEA)", x = "tSNE_1", y = "tSNE_2", color = "") +
  ggplot2::scale_color_manual(name = "", values = mycolor) +
  ggplot2::theme_classic(base_size = 20) +
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, size = 18)) +
  ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 4),
                                                 ncol = 1))
...
<img src="figures/figure_10_0300.png" width="200px">
<img src="figures/figure_10_0305.png" width="200px">

Investigate “significant” modules for ssGSEA clustering results.

``` r
set.seed(1)
temp <- Seurat::CreateSeuratObject(counts = t(pdac@misc[["enrichIt"]]))
temp <- Seurat::AddMetaData(temp, metadata = pdac$ssgsea_clusters,
                            col.name = "ssgsea_clusters")
temp <- Seurat::SetIdent(temp, value = "ssgsea_clusters")
res <- Seurat::FindAllMarkers(temp, only.pos = TRUE,
                              min.pct = 0.20, logfc.threshold = 0.20)
rownames(res) <- seq_len(nrow(res))
pdac@misc[["ssGSEA_stat"]] <- res
```

Below are significant modules for ssGSEA clusters.

    0: GOBP-REVERSE-CHOLESTEROL-TRANSPORT (p_val_adj ~ e-224)
    1: GOBP-SIGNAL-PEPTIDE-PROCESSING (p_val_adj ~ e-143)
       GOBP-REGULATION-OF-TYPE-B-PANCREATIC-CELL-DEVELOPMENT (p_val_adj ~ e-88)
    2: GOBP-REGULATION-OF-MACROPHAGE-FUSION (p_val_adj ~ e-116)
    3: GOBP-KERATINOCYTE-DEVELOPMENT (p_val_adj ~ e-44)
    4: GOBP-REGULATION-OF-INTEGRIN-MEDIATED-SIGNALING_PATHWAY (p_val_adj ~ e-52)

``` r
tmp <- as.integer(as.character(pdac$ssgsea_clusters))
pdac$cell_state <- tmp
pdac$cell_state[pdac$cell_state == 0] <- "RCT"
pdac$cell_state[pdac$cell_state == 1] <- "SPP"
pdac$cell_state[pdac$cell_state == 2] <- "RMF"
pdac$cell_state[pdac$cell_state == 3] <- "KD"
pdac$cell_state[pdac$cell_state == 4] <- "RIS"
```
...
``` r
title <- "PDAC (ssGSEA)"
surt_st <- pdac[, grepl("ST", pdac$batch)]
lv <- c("RCT", "SPP", "RMF", "KD", "RIS")
labels <- factor(surt_st$cell_state, levels = lv)
mycolor <- c("SPP" = "limegreen", "IRS" = "grey20")
p <- Spaniel::spanielPlot(object = surt_st, grob = surt_st@images[[1]],
                          plotType = "Cluster", clusterRes = "cell_state",
                          ptSize = 2.5, customTitle = "PDAC-A (ssGSEA)") +
  ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 4))) +
  ggplot2::scale_color_manual(name = "Cell state", values = mycolor) +
  ggplot2::theme_void() +
  ggplot2::theme(text = ggplot2::element_text(size = 18, family = "Helvetica"))
filename <- "figures/figure_10_0310.png"
ggplot2::ggsave(file = filename, plot = p, dpi = 50, width = 5.05, height = 4.05)
```

<img src="figures/figure_10_0310.png" width="300px">
...
``` r
temp <- pdac[[]]
labels <- temp$cell_state
labels[which(labels %in% c("RCT", "RMF", "KD"))] <- "Others"
labels[which(labels %in% c("SPP"))] <- "Pancreas"
labels[which(labels %in% c("RIS"))] <- "Cancer"
labels <- factor(labels, levels = c("Pancreas", "Cancer", "Others"))
descriptions <- c(gsub("-", "_", "GOBP-SIGNAL-PEPTIDE-PROCESSING"),
                  gsub("-", "_", "GOBP-REGULATION-OF-INTEGRIN-MEDIATED-SIGNALING_PATHWAY"))
titles <- descriptions
mat <- pdac@misc[["enrichIt"]]
...
for(i in seq_along(descriptions)){
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::labs(title = titles[i], x = "ssGSEA annotations",
                  y = "ssGSEA scores") +
    ggplot2::scale_x_discrete(guide = ggplot2::guide_axis(angle = 45)) +
    ggplot2::theme_classic(base_size = 20) +
    ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, size = 15))
  filename <- sprintf("figures/figure_10_%04d.png", 319 + i)
  ggplot2::ggsave(file = filename, plot = p, dpi = 50, width = 6, height = 4)
}
```

<img src="figures/figure_10_0320.png" width="200px">
<img src="figures/figure_10_0321.png" width="200px">

``` r
# Save data.
saveRDS(pdac, file = "backup/10_031_pdac_ssGSEA_gobp.rds")

# Load data.
pdac <- readRDS("backup/10_031_pdac_ssGSEA_gobp.rds")
```
...
Taking an intersection of genes between inDrop and ST datasets,
concatenate the read count tables of these data using `bayNorm` with
`Prior_type = "GG"` for attenuating batch effects.

``` r
genes <- intersect(rowData(pdacrna)$gene, rowData(pdacst)$gene)
pdacrna <- pdacrna[genes, ]
pdacst <- pdacst[genes, ]
identical(rowData(pdacrna)$gene, rowData(pdacst)$gene)
```
...
``` r
cmat <- cbind(as.matrix(assay(pdacrna, "counts")),
              as.matrix(assay(pdacst, "counts")))
cond <- colnames(cmat)
names(cond) <- colnames(cmat)
cond[grepl("inDrop", cond)] <- "inDrop"
cond[grepl("ST", cond)] <- "ST"
res <- bayNorm::bayNorm(Data = cmat, Conditions = cond, Prior_type = "GG",
                        mode_version = TRUE)
```
...
``` r
bmat <- cbind(res[["Bay_out_list"]][["Group inDrop"]],
              res[["Bay_out_list"]][["Group ST"]])
pdac <- pagoda2::Pagoda2$new(x = bmat, modelType = "plain", n.cores = 1,
                             log.scale = TRUE, min.cells.per.gene = 0,
                             batch = cond)
```

    2462 cells, 7923 genes; normalizing ... 
    Using plain model 
    Batch ... 
    log scale ... 
    done.
...
``` r
filename <- "figures/figure_10_0400.png"
png(file = filename, height = 300, width = 600, res = 50)
pdac$adjustVariance(plot = TRUE)
dev.off()
```

    calculating variance fit ...
     using gam 
    1065 overdispersed genes ... 1065
    persisting ... 
     using gam 
    done.

<img src="figures/figure_10_0400.png" width="400px">

Perform principal component analysis, where `n.odgenes` is the number of
overdispersed genes.

``` r
pdac$calculatePcaReduction(nPcs = 50, n.odgenes = 1065)
```

    running PCA using 1065 OD genes .
    .
    .
    .
     done
...
pdac$testPathwayOverdispersion(setenv = go.env, verbose = TRUE,
                               correlation.distance.threshold = 0.8,
                               min.pathway.size = 5, max.pathway.size = 2000,
                               recalculate.pca = FALSE)
```

    determining valid pathways
    processing 9024 valid pathways
    scoring pathway od signifcance
    compiling pathway reduction
    clustering aspects based on gene loading ... 422 aspects remaining
    clustering aspects based on pattern similarity ... 65 aspects remaining

Create a Seurat object inputting PAGODA2 weighted PC score matrix,
keeping ST spot information.

``` r
# Create a Seurat object.
mat <- pdac[["misc"]][["pathwayOD"]][["xv"]]
surt <- Spaniel::createSeurat(counts = mat, barcodeFile = fn,
                              projectName = "PDAC-A", sectionNumber = "1")
surt@images <- metadata(pdacst)$images
...
``` r
mat <- as.matrix(Seurat::GetAssayData(object = surt, slot = "data"))

# t-SNE
set.seed(1)
res <- Rtsne::Rtsne(t(mat), dim = 2, pca = TRUE, initial_dims = 50)
surt@reductions[["tsne_pagoda2"]] <- res$Y

# UMAP
set.seed(1)
res <- umap::umap(t(mat), n_components = 2)
surt@reductions[["tsne_pagoda2"]] <- res$layout
```
...
``` r
mat <- as.matrix(Seurat::GetAssayData(object = surt, slot = "data"))
temp <- Seurat::CreateSeuratObject(counts = mat, project = "pdac")
temp <- Seurat::ScaleData(temp, features = rownames(temp))
temp <- Seurat::RunPCA(temp, features = rownames(temp))
temp <- Seurat::FindNeighbors(temp, reduction = "pca", dims = seq_len(40))
temp <- Seurat::FindClusters(temp, resolution = 0.20)
surt <- Seurat::AddMetaData(surt, metadata = temp$seurat_clusters,
                            col.name = "pagoda2_clusters")
```

Show the clustering results in low-dimensional spaces.

``` r
# PAGODA2 clustering result
labels <- surt$pagoda2_clusters
mycolor <- c("0" = "deepskyblue1", "1" = "grey20", "2" = "red", "3" = "grey80",
             "4" = "orange")
df <- as.data.frame(surt@reductions[["tsne_pagoda2"]])
p <- ggplot2::ggplot() +
  ggplot2::geom_point(ggplot2::aes(x = df[, 1], y = df[, 2], color = labels),
                      size = 0.2, alpha = 1) +
  ggplot2::labs(title = "PDAC (PAGODA2 function space)",
                x = "tSNE_1", y = "tSNE_2", color = "") +
  ggplot2::scale_color_manual(name = "", values = mycolor) +
  ggplot2::theme_classic(base_size = 20) +
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, size = 18)) +
  ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 4),
                                                 ncol = 1)) + ggplot2::xlim(-5, 5)
...
<img src="figures/figure_10_0410.png" width="200px">
<img src="figures/figure_10_0415.png" width="200px">

On the other hand, cell states are also embedded in a gene expression
space, which can be visualized by t-SNE plot.

``` r
# PAGODA2 clustering result
set.seed(1)
pdac$getEmbedding(type = 'PCA', embeddingType = 'tSNE', perplexity = 30,
                  verbose = FALSE)
df <- pdac[["embeddings"]][["PCA"]][["tSNE"]]
labels <- surt$pagoda2_clusters
mycolor <- c("0" = "deepskyblue1", "1" = "grey20", "2" = "red", "3" = "grey80",
             "4" = "orange")
p <- ggplot2::ggplot() +
  ggplot2::geom_point(ggplot2::aes(x = df[, 1], y = df[, 2], color = labels),
                      size = 0.2, alpha = 1) +
  ggplot2::labs(title = "PDAC (PAGODA2 gene space)",
                x = "tSNE_1", y = "tSNE_2", color = "") +
  ggplot2::scale_color_manual(name = "", values = mycolor) +
  ggplot2::theme_classic(base_size = 20) +
  ggplot2::theme(plot.title = ggplot2::element_text(hjust = 0.5, size = 18)) +
  ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 4),
                                                 ncol = 1))
filename <- "figures/figure_10_0420.png"
ggplot2::ggsave(file = filename, plot = p, dpi = 50, width = 5.1, height = 4.3)
...
p <- ggplot2::ggplot() +
  ...
  ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 4),
                                                 ncol = 1))
filename <- "figures/figure_10_0425.png"
ggplot2::ggsave(file = filename, plot = p, dpi = 50, width = 5.6, height = 4.3)
```

<img src="figures/figure_10_0420.png" width="200px">
<img src="figures/figure_10_0425.png" width="200px">

Investigate “significant” aspect for PAGPDA2 clustering results.

``` r
mat <- as.matrix(Seurat::GetAssayData(object = surt, slot = "data"))
temp <- Seurat::CreateSeuratObject(counts = mat)
temp <- Seurat::AddMetaData(temp, metadata = surt$pagoda2_clusters,
                            col.name = "pagoda2_clusters")
temp <- Seurat::SetIdent(temp, value = "pagoda2_clusters")
res <- Seurat::FindAllMarkers(temp, only.pos = TRUE,
                              min.pct = 0.20, logfc.threshold = 0.20)
rownames(res) <- seq_len(nrow(res))
surt@misc[["pagoda2_stat_function"]] <- res
```

Below are significant modules for PAGODA2 clusters.

    0: aspect9 (p_val_adj ~ e-205)
    1: aspect14 (p_val_adj ~ e-221)
    2: aspect4 (p_val_adj ~ e-122)
    3: aspect31 (p_val_adj ~ e-93)
    4: aspect3 (p_val_adj ~ e-34)

Investigate significant genes for PAGPDA2 clustering results.

``` r
# Return variance-normalized matrix.
mat <- t(as.matrix(pdac$getNormalizedExpressionMatrix()))
temp <- Seurat::CreateSeuratObject(counts = mat)
temp <- Seurat::AddMetaData(temp, metadata = surt$pagoda2_clusters,
                            col.name = "pagoda2_clusters")
temp <- Seurat::SetIdent(temp, value = "pagoda2_clusters")
res <- Seurat::FindAllMarkers(temp, only.pos = TRUE,
                              min.pct = 0.20, logfc.threshold = 0.20)
rownames(res) <- seq_len(nrow(res))
surt@misc[["pagoda2_stat_expression"]] <- res
```

Below are significant genes for PAGODA2 clusters. Defining significant
genes as genes with p_val_adj\<1e-100, infer cell states using
[GeneCards](https://www.genecards.org/).

    0: Pancreatic acinar cell # DMBT1 (p_val_adj ~ e-119)
    1: Cancer cell            # COL17A1 (p_val_adj ~ e-180), S100P (p_val_adj ~ e-113)
    2: Blood cell             # SRGN (p_val_adj ~ e-293), CD37 (p_val_adj ~ e-198)
    3: Unspecified            # No significant genes were detected.
    4: Pancreatic cell        # CELA2B (p_val_adj ~ e-264), CELA2A (p_val_adj ~ e-250)

``` r
tmp <- as.integer(as.character(surt$pagoda2_clusters))
surt$cell_state <- tmp
surt$cell_state[surt$cell_state == 0] <- "Acinar"
surt$cell_state[surt$cell_state == 1] <- "Cancer"
surt$cell_state[surt$cell_state == 2] <- "Blood"
surt$cell_state[surt$cell_state == 3] <- "Unspecified"
surt$cell_state[surt$cell_state == 4] <- "Pancreas"
```
...
title <- "PDAC (PAGODA2)"
surt_st <- surt[, grepl("ST", surt$batch)]
lv <- c("Acinar", "Cancer", "Blood", "Unspecified", "Pancreas")
labels <- factor(surt_st$cell_state, levels = lv)
mycolor <- c("Acinar" = "deepskyblue1", "Cancer" = "grey20", "Blood" = "red",
             "Unspecified" = "grey80", "Pancreas" = "orange")
p <- Spaniel::spanielPlot(object = surt_st, grob = surt_st@images[[1]],
                          plotType = "Cluster", clusterRes = "cell_state",
                          ptSize = 2.5, customTitle = "PDAC-A (PAGODA2)") +
  ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 4))) +
  ggplot2::scale_color_manual(name = "Cell state", values = mycolor) +
  ggplot2::theme_void() +
  ggplot2::theme(text = ggplot2::element_text(size = 18, family = "Helvetica"))
...
``` r
# Save data.
saveRDS(surt, file = "backup/10_043_pdac_pagoda2_seurat.rds")

# Load data.
surt <- readRDS("backup/10_043_pdac_pagoda2_seurat.rds")
```
