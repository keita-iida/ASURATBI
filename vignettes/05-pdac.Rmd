...
## ssGSEA using Gene Ontology database
Load the [data after quality control](#data_qc) and ST spot information.
```{r, eval = FALSE, echo = FALSE}
pdacrna <- readRDS("backup/08_002_pdacrna_dataqc.rds")
pdacst <- readRDS("backup/09_002_pdacst_dataqc.rds")
...
Perform t-distributed stochastic neighbor embedding and
...
## PAGODA2 using Gene Ontology database
Load the [data after quality control](#data_qc) and ST spot information.
```{r, eval = FALSE, echo = FALSE}
pdacrna <- readRDS("backup/08_002_pdacrna_dataqc.rds")
pdacst <- readRDS("backup/09_002_pdacst_dataqc.rds")
...
Taking an intersection of genes between inDrop and ST datasets, concatenate the
read count tables of these data using `bayNorm` with `Prior_type = "GG"`
for attenuating batch effects.
```{r, eval = FALSE}
...
```{r, eval = FALSE, echo = FALSE}
# Save data.
saveRDS(pdac, file = "backup/10_041_pdac_pagoda2.rds")

# Load data.
pdac <- readRDS("backup/10_041_pdac_pagoda2.rds")
```
...
```{r, eval = FALSE, echo = FALSE}
# Save data.
saveRDS(pdac, file = "backup/10_042_pdac_testPathwayOverdispersion.rds")

# Load data.
pdac <- readRDS("backup/10_042_pdac_testPathwayOverdispersion.rds")
```
...
On the other hand, cell states are also embedded in a gene expression space,
which can be visualized by t-SNE plot.
```{r, eval = FALSE}
# PAGODA2 clustering result
set.seed(1)
...
Below are significant genes for PAGODA2 clusters.
Defining significant genes as genes with p_val_adj<1e-100, infer cell states
using [GeneCards](https://www.genecards.org/).
```
